...
class CardUtils {
  ...
  static String concatenateCardFields(PaymentCard? card) {
    ...
    final String? number = StringUtils.nullify(card.number);
    final String? cvc = StringUtils.nullify(card.cvc);
    final int expiryMonth = card.expiryMonth ?? 0;
    final int expiryYear = card.expiryYear ?? 0;
